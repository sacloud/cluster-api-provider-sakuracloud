...
# TODO あとで確認する
REGISTRY ?= sacloud
STAGING_REGISTRY := staging-cluster-api-sakuracloud
PROD_REGISTRY := cluster-api-sakuracloud
IMAGE_NAME ?= cluster-api-sakuracloud-controller
...
.PHONY: manager
manager: generate
	go build -o $(BIN_DIR)/manager -ldflags '-extldflags "-static" -w -s'
...
create-management-cluster: ## Create a development Kubernetes cluster on SakuraCloud in a KIND management cluster.
	kind create cluster --name=clusterapi

.PHONY: delete-management-cluster
delete-management-cluster: ## Destroys the "clusterapi" kind cluster.
