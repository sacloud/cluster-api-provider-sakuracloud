...
func main() {
	...
	record.InitFromRecorder(mgr.GetEventRecorderFor("sakuracloud-controller"))

	if err = (&controllers.SakuraCloudMachineReconciler{
		Client:   mgr.GetClient(),
		Log:      ctrl.Log.WithName("controllers").WithName("SakuraCloudMachine"),
		Recorder: mgr.GetEventRecorderFor("sakuracloudmachine-controller"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "SakuraCloudMachine")
		os.Exit(1)
	}
	if err = (&controllers.SakuraCloudClusterReconciler{
		Client:   mgr.GetClient(),
		Log:      ctrl.Log.WithName("controllers").WithName("SakuraCloudCluster"),
		Recorder: mgr.GetEventRecorderFor("sakuracloudcluster-controller"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "SakuraCloudCluster")
		os.Exit(1)
	}
